...
impl DiscordWebhook {
    pub async fn send(&self, ping: &Ping) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.client.as_ref();
        client.expect("No Client Found").send(|message| message
            .content(DiscordWebhook::format(&self.content, ping).as_str())
            .embed(|embed| embed
            .description(DiscordWebhook::format(&self.embed_content,  ping).as_str())))
            .await
    }

    fn format(string: &str, ping: &Ping) -> String {
